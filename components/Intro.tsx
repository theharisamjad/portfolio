import React from 'react';
import getConfig from 'next/config'
import Link from 'next/link'

const { publicRuntimeConfig } = getConfig()

interface IntroProps {
  title: string;
  description: string;
  image: string;
  buttons: { isPrimary: boolean; link: string; title: string }[];
}

export const Intro: React.FC<IntroProps> = ({ title, description, image, buttons }) => {
  return (
    <div className="bg-secondary py-5 px-5">
      <div className="container">
        <div className=" row align-items-center">
          <div className="col-sm-6">
            <h1 className="text-primary fw-bold display-3">{title}</h1>
            <p>{description}</p>
            <div className="text-center">
              {buttons.map((value, index) => (
                (value.isPrimary) ?
                  <Link key={index} href={value.link}>
                    <button className="btn btn-primary my-1 mx-3">{value.title}</button>
                  </Link>
                  :
                  <Link key={index} href={value.link} target="_blank" rel="noreferrer">
                    <button className="btn btn-outline-primary my-1 mx-3">{value.title}</button>
                  </Link>
              ))}
            </div>
          </div>
          <div className="col-sm-6 text-center">
            <img
              className="img-fluid my-3 card-image" width="250"
              height="250" src={image}
              alt="profile of hashirshoaeb"
            />
          </div>
        </div>
      </div>
    </div>
  );
}

interface AboutProps {
  title: string;
  description: string[];
}

export const About: React.FC<AboutProps> = ({ title, description }) => {
  return (
    <div id="about" className="bg-white py-5 px-5">
      <div className="container">
        <h1 className="text-primary fw-bold">{title}</h1>
        <div className="px-sm-5">
          {description.map((value, index) => (
            <p key={index} >{value}</p>
          ))}
        </div>
      </div>
    </div>
  );
}